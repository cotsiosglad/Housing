...
const ProjectDetails = ({ match }) => {
  ...
  async function fetchAndSetProjectImages(path) {
    try {
      ...
      if (files.length > 0) {
        const newProjectImages = [...projectImages, { files, flat: '', destinationFolder: 'projectImages' }];
  ...
  async function fetchAndsetApartmentUploadedFiles(path) {
    try {
      ...
      const files = fileData.map((m) => ConvertPathToGalleriaModel(m.fileUrl, m.fileName));
      if (files.length > 0) {
        const newProjectImages = [...projectImages, { files, flat: '', destinationFolder: 'apartments' }];
    ...
    GetDocById(projectRef, "Projects", "refName").then(async (projectData) => {
      ...
      GetDocById(projectData[0].id, "ProjectApartments", "projectId").then(async (data) => {
        ...
        data[0].apartments.map(async (item) => {
          path = "projects/" + projectData[0].refName + "/apartments/" + item.flatNo
  ...
  const avav = () => {
    return <ProjectDetailsPreview project={project} projectMainImage={projectMainImage} proejctDescription={textEditorValue} apartmentList={projectApartmentList} projectDocuments={projectDocuments} projectImages={projectImages} projectSideImage={projectSideImage} />
