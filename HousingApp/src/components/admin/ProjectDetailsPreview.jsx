...
const ProjectDetailsPreview = ({ project, apartmentList, proejctDescription, projectMainImage, projectImages, projectDocuments, projectSideImage }) => {
    ...
    async function fetchAndSetItemImages(path) {
        try {
            ...
            const files = fileData.map((m) => { return ConvertPathToGalleriaModel(m.fileUrl, m.fileName) });
            if (files.length > 0) {
    ...
    useEffect(() => {
        // debugger;
        let _images = projectImages.length > 0 ? projectImages : [{
    ...
    const onSelectItem = (item) => {
        ...
        fetchAndSetItemImages(path).then((res) => {
            debugger;
    ...
    return (
        <>
            ...
            <Galleria ref={galleria}
                ...
                responsiveOptions={responsiveOptions} numVisible={9} style={{ maxWidth: '50%' }}
          ...
          <img src={background} alt='' />
                        ...
                        <div className='project-wrapper-box'>
                            ...
                            <div className='project-wrapper-content'>
                                <div className='project-content-text'>
                                    <div dangerouslySetInnerHTML={{ __html: proejctDescription }} />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <img src={val} alt="" key={index} onClick={() => setToggler(!toggler)}/>
                            ...
                            <div className='project-details-card'>
                                <ImageCarousel imageList={projectImages.map(m => m.files).flat()} />
                                {/* <ImageViewer imageList={project.images} /> */}
                            </div>
              ...
              /> */}
                ...
                <div className={`row ${apartmentList && apartmentList.length>0?'':'d-none'}`}>
